<template>
  <div
    class="!tw-ring tw-ring-primary/60 !tw-rounded-lg sm:tw-w-[600px] tw-relative tw-bg-white"
  >
    <h3 class="tw-font-mono tw-text-lg tw-p-3">
      {{
        isTrigger
          ? `${
              step?.name && activeStepStage === "step_form"
                ? step?.name
                : "Trigger"
            }`
          : `${
...
const emits = defineEmits<{
  (e: "add-step", value?: StepOutput[]): void;
}>();
